...
The first is the method `Const` and the type `Constant`. If a property on the value to be transformed is a URI or represents a URI, it can be passed directly to the method `Const`. This will directly produce a link relation with the specified relation and with `href` set to the specified value. This can be seen on line 15 of the example.

```csharp
.Link(_pool, pj => Const(pj.Pool))
```

If other link properties are required, fall back to calling the constructor for `Constant` and set the desired properties directly, as seen on line 39 of the example.
...
This can be seen on line 16 of the example.
...
`Link` has overloads with special handling for types implementing `IEnumerable<T>` or `IDictionary<TKey, TValue>`. Line 39 of the example shows this in use.

```csharp
.Link(_assets, i => i.Assets, a => new Constant(a.Uri) { Title = a.Role })
```

For this API, the value associated with the link relation `https://relations.fen.cimpress.io/watchers` should be a collection of links. This is different from line 16, which creates only one link.
...
If a value is used for link creation – as `pj.Id` is on line 14 of the example – it's unlikely to be useful to a client in the body of the value.

```csharp
.Self<PrintJob>(pj => Route("GetByPrintJobId", new { pj.Id }))
```

Once a client has requested `application/hal+json`, they should no longer have a need to glue together `baseURI`s and `ID`s (or whatever) in order to synthesize a URI to a resource. The method `Ignore` will remove the selected property (key and value) from the serialization. Many properties can be specified in a single call to `Ignore`, as on line 19 of the example.
